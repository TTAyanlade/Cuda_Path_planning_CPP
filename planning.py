...
from geometry import (get_euclidean_distance, get_euclidean_distance_gpu)
import datetime
...
class EuclideanDistanceComputator_gpu(DistanceComputator):
    def get_distance(self, s1, s2):
        """Return the Euclidean distance between s1 and s2"""
        return get_euclidean_distance_gpu(s1, s2)
...
def rrt(
    ...
    for i in range(numIt):
        iter_time.append(datetime.datetime.now())
        print(i)
        ...
        if dist > tol:
            ...
            if use_goal and get_euclidean_distance(qs, qG) < tol:
                return (G, root, vs, iter_time)

    return (G, root, None, iter_time)
...
def rrt_gpu(
    ...
    for i in range(numIt):
        ...
        if use_goal:
            ...
            alpha = sample(cspace)
        vn = G.get_nearest_gpu(alpha, distance_computator, tol)
        qn = G.get_vertex_state(vn)
        (qs, edge) = stopping_configuration(
            qn, alpha, edge_creator, collision_checker, tol
        )
...
def rrt_star(
    ...
    for i in range(numIt):
        ...
        print(i) 
        use_goal = qG is not None and random.uniform(0, 1) <= pG
        if use_goal and path_found == False:
            alpha = np.array(qG)
        else:
            alpha = sample(cspace)
        # vn = G.get_nearest(alpha, distance_computator, tol)
        vn = G.get_nearest_vertex(alpha, distance_computator, k, root)
        qn = G.get_vertex_state(vn)
        (qs, edge) = stopping_configuration(
            qn, alpha, edge_creator, collision_checker, tol
        )
        ...
        #rewire old nodes if cost is lower
        G.rewire_edges_straight(vs, k, distance_computator, root, rho_min, edge_creator, collision_checker, tol)
    if path_found == True:
        return (G, root, goal_vertex, iter_time)
    else:
        return(G, root, None, iter_time)
...
def rrt_star_gpu(
    ...
    for i in range(numIt):
        ...
        if use_goal and path_found == False:
            ...
            alpha = sample(cspace)
        # vn = G.get_nearest(alpha, distance_computator, tol)
        vn = G.get_nearest_vertex_gpu(alpha, distance_computator, k, root)
        qn = G.get_vertex_state(vn)
        (qs, edge) = stopping_configuration(
            qn, alpha, edge_creator, collision_checker, tol
        )
        ...
        #rewire old nodes if cost is lower
        G.rewire_edges_straight_gpu(vs, k, distance_computator, root, rho_min, edge_creator, collision_checker, tol)
    if path_found == True:
        return (G, root, goal_vertex, iter_time)
    else:
        return(G, root, None, iter_time)
